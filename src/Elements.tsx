...
import AnnotatedText, {AnnotatedCharacter, TranslatedLine} from "./AnnotatedText";
...
export default class Elements extends Component<Props, State> {
  ...
  elementContent(element: [string, string], rootSet: Set<string>) {
    ...
    if (translation == undefined) {
      ...
      annotatedChar = <AnnotatedCharacter root={ROOTS.get(elementRoot)!} displaySettings={this.props.displaySettings}/>;
    ...
    return (
      <>
        ...
        <p>
          ...
          In many cases, roots were selected by calquing existing etymologies (e.g., oxygen from Greek <i>ὀξύς</i> "sharp, sour").
          ...
          I've avoided merely phonetic matching, even when elements are named for proper nouns with no clear further etymology.
          For cases where calquing existing etymologies is not possible, either because the element is named for a proper noun,
          ...
          softness, tin for the crackling <a href="https://en.wikipedia.org/wiki/File:Zinnschrei.webm" target="_blank">"tin cry"</a>)
          ...
          elements at time of writing, and 366 Jaobon roots, so there should be plenty of headroom to pick fitting element names.{' '}
            ...
            <tr key={i}>
              {row.map((element, j) => (
                <td key={j} style={{padding: 0, border: (element === null ? "none" : "1px solid black")}}>
                  {element !== null && this.elementContent(element, rootSet)}
                </td>
              ))}
            </tr>
          ))}
